...
## 2026-06-17 00:20 - Create readme.md prompt

#### Tool: chatGPT GPT-5.5

#### Prompt:
A README with exact, step-by-step commands to: install dependencies, start the
database, run migrations, run the seed, run the tests, and start the API and the UI.  this is my requirments for the app readme. help me craetea a prompt for claude to make him write my app's readme. it suppose to be detailed and clean and clear. he need to explain how to run the db (docker compose file) how to run the server and the front. the tests, the .env that people need to set up... and every other thing that seems important to doc in the app readme for usage.

make the prompt short (like one peragraph) thx

#### Result:
Write a clean, detailed, production-ready README for my full-stack app. It should include exact step-by-step commands for everything needed to run the project from scratch: installing dependencies, setting up environment variables (.env with all required keys explained), starting the database using Docker Compose (include how to start/stop/reset it), running database migrations, running seed scripts, starting the backend API server, starting the frontend UI, and running tests (unit/integration/e2e if available). Assume a typical modern stack (Node.js, TypeScript, React frontend, backend API, database). The README should be structured, easy to follow, and written for a new developer joining the project. Include prerequisites, project structure overview, common troubleshooting tips, and any important notes about development vs production modes. Make sure all commands are explicit and copy-paste ready, and ensure nothing important for running the project is missing.
